...
1. How is the client sending data to the server? What protocol?
      TCP (SOCK_STREAM on AF_INET). The client calls connect() and write() on
the socket. It sends to 127.0.0.1:8001.
2. What data is the client sending to the server?
      It sends five strings: "Hello", "Apple", "Car", "Green", and "Dog".
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
      It receives a pointer to a struct acceptor_args which contains: run,
list_handle, and list_lock.
2. How are received messages stored?
      In a singly linked list. Each message gets a heap-allocated struct
list_node with a void *data.
3. What does `main()` do with the received messages?
      main() calls collect_all(head), which traverse the list, prints each
message, frees the node and its data, and returns the count.
4. How are threads used in this sample code?
      The main thread sets up the list/mutex, spawns the acceptor thread, waits
for enough messages, then joins the acceptor. run_acceptor listens on the server
socket in a loop; for each incoming connection, it will then spawn a client
thread. run_client reads from its own socket and appens received messages to the
shared list.

Part 5: Non-blocking sockets:
  - How are sockets made non-blocking?
      set_non_blocking(), which uses fcntl(fd, F_GETFL) to read the current
flags and fcntl(fd, F_SETFL, flags | O_NONBLOCK) to add O_NONBLOCK.
  - What sockets are made non-blocking?
      The server listening socket (sfd) in run_acceptor(), and each accepted
client socket (cfd) in run_client().
  - Why are these sockets made non-block? What purpose does it serve?
      Without O_NONBLOCK, accept() and read() would block indefinitely waiting
for a connection or data. By making them non-blocking, those calls return
immediately with EAGAIN/EWOULDBLOCK when nothing is available.
*/
